---
title: "Energia Elettrica in Italia"
author: "Manuel Scubla"
output:
...
gconspefos = ggplot( consumofossilenetto %>% 
   ...
   theme( axis.text.x = element_text( angle = 45, size = 5 ), 
          legend.background = element_rect( fill = "lightgrey",
                                           size = 0.5, linetype = "solid", 
                                           colour = "black") ) +
   labs( title = "CONSUMI DI COMBUSTIBILE FOSSILE PER LA PRODUZIONE DI ENERGIA ELETTRICA" )
...
gcontotfos = ggplot( consumofossilenetto %>% 
   ...
   theme( axis.text.x = element_text( angle = 45, size = 5 ), 
          legend.background = element_rect( fill = "lightgrey",
                                           size = 0.5, linetype = "solid", 
                                           colour = "black") ) +
   labs( title = "CONSUMI TOTALI DI COMBUSTIBILE FOSSILE PER LA PRODUZIONE DI ENERGIA ELETTRICA" )
```

## Indice
### Analisi 1997-2019:
* Polazione residente
* Famiglie allacciate alla rete elettrica
* Consumi di energia elettrica
* Produzione di energia totale elettrica
* Inquinamento
* Prezzi
...
ggarrange( gpopolazione, gconene, gfamiglie, ncol = 2, nrow = 2 ) 
```

## 

```{r echo = FALSE}
p = paste( "*Per termoelettrica tradizionale si intende solidi, gas naturale, gas derivati" )
p = ggparagraph( text = p, face = "italic", size = 12, color = "black" )

# relazione produzione e consumo:
ggarrange( gprofon, gprotot, ncol = 1, nrow = 2 )
ggarrange( gconene, p, ncol = 1, nrow = 2 )
```
...
ggarrange( ganisolfo, gossazo, ncol = 1, nrow = 2 )
ggarrange( ggasserra, ncol = 1, nrow = 2 )
```
...
```{r echo = FALSE}
# relazione prezzi:
ggarrange( gpreele, gpregas, ncol = 1, nrow = 2 )
ggarrange( gprepro, ncol = 1, nrow = 1 )
```
...
ggarrange( gprototline, gconene,ncol = 1, nrow = 2 )
ggarrange(  gdipene, ncol = 1, nrow = 2 ) 
```
...
ggarrange( gconspefos, ncol = 1, nrow = 1 )
ggarrange( gprepro, ncol = 1, nrow = 1 )
ggarrange( gcontotfos, ggasserraline, ncol = 1, nrow = 2 )
ggarrange( gprofon, ncol = 1, nrow = 2 )
```
...
ggarrange( gpreele, gpregas, ncol = 1, nrow = 2 )
ggarrange( grelconpro, gprofon, ncol = 1, nrow = 2 )
ggarrange( gdipene, ncol = 1, nrow = 2 )
